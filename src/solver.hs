...
main = do
	...
	let filename = "/home/ponaszki/workspace/haskell/SPOP-puzzle-solver/src/p2.txt"
	filename <- getLine  
	putStrLn ("Opening: " ++ filename )

	--[s] <- getArgs --to do wczytywania pliku jako argument wywołania aplikacji
	--putStrLn (s)
	--handle <- openFile  ReadMode 
...
solvePuzzle' rss css m 	|isAllMatrixFilled m  = m
								|otherwise = checkAllRows rss 1 (checkAllColumns css 1 m)
...
--isRowFullfilled :: Int-> [Int] ->Matrix Int -> Bool
--isRowFullfilled _ [] m = True
--isRowFullfilled  rowNr (x:xs) m  	| sumList (x:xs) == length (getElementsToPaintInRow  x:xs (ncols m)) = True
--							| otherwise = False

--excludeFromFullRow :: Int ->  
